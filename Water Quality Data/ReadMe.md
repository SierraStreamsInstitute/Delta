# Read Me
#### Water Quality Data
#### Authors: Melissa Grim and Jessica Herrmann
#### Updated: 13 May 2026 by Jessica Herrmann

## I. Background
This folder contains water quality data for SSI's Delta Project field sites, as well as surface water chemistry data for project field and supplemental sites, obtained from the California Environmental Data Exchange Network (CEDEN) and CEDEN Augment. The CEDEN Augment is water chemistry data (e.g. Nitrate, Phosphate, AFDM, and Chlorophyll) which are not also in CEDEN’s Surface Water Chemistry dataset.

Data for any given year can be added at any time, e.g. we uploaded our water quality data for 2020 through 2024 in 2025, so we may consider re-downloading prior data years again in 2027 when we pull the data from 2026. Because this data comes from multiple sources, e.g. USFS, State, and monitoring groups like ours, it is worth reading through the metadata documents such as the CEDEN Water Chemistry Data Dictionary.

## II. SSI Field Sites Water Quality Data
### 

## III. CEDEN Surface Water Chemistry Datasets
### 
